          ...
          <div class="bg-card border shadow-lg rounded-lg max-w-lg w-full p-6 space-y-6">
            ...
            <form [formGroup]="providerForm" (ngSubmit)="onSubmit()" class="space-y-4">
              ...
              <div [ngSwitch]="providerForm.get('type')?.value" class="space-y-4 pt-2 border-t">
                ...
                <div *ngSwitchCase="'CLOUDFLARE'" class="space-y-4">
                  <div class="space-y-2">
                    ...
                    <input class="input" formControlName="apiToken" type="password" placeholder="Cloudflare API Token">
                  </div>
                  <div class="space-y-2">
                ...
                <div *ngSwitchCase="'AWS_ROUTE53'" class="space-y-4">
                  ...
                  <div class="space-y-2">
                    ...
                    <input class="input" formControlName="secretKey" type="password" placeholder="Secret Key">
                  </div>
                </div>
                 ...
                 <div *ngSwitchCase="'GODADDY'" class="space-y-4">
                  ...
                  <div class="space-y-2">
                    ...
                    <input class="input" formControlName="apiSecret" type="password" placeholder="Secret">
                  </div>
                </div>
              ...
              <div class="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" class="btn-primary" [disabled]="providerForm.invalid">Save Provider</button>
              </div>
            </form>
          </div>
        </div>
  ...
  onSubmit(): void {
    if (this.providerForm.valid) {
      ...
      if (val.type === 'CLOUDFLARE') {
        config.credentials = { apiToken: val.apiToken, zoneId: val.zoneId };
      } else if (val.type === 'AWS_ROUTE53') {
        config.credentials = { accessKey: val.accessKey, secretKey: val.secretKey };
      } else if (val.type === 'GODADDY') {
        config.credentials = { apiKey: val.apiKey, apiSecret: val.apiSecret };
      }

      this.providerService.createProvider(config).subscribe(() => {
